...
import 'package:taxi_app_flutter/models/route.dart';
import 'package:taxi_app_flutter/services/map_requests.dart';
import 'package:taxi_app_flutter/services/ride_request.dart';
import 'package:taxi_app_flutter/services/rider.dart';
...
//         onResume: handleOnResume);
    ...
    FirebaseMessaging.onMessage.listen((event) {
      _handleNotificationData1(event);
    });
  ...
  _userCurrentLocationUpdate(Position updatedPosition) async {
    ...
    if (distance >= 50) {
      if(show == Show.RIDER){
        sendRequest(coordinates: requestModelFirebase.getCoordinates(), intendedLocation: '');
      }
...
// ANCHOR PUSH NOTIFICATION METHODS
  ...
  _handleNotificationData1(RemoteMessage data) async {
    hasNewRideRequest = true;
    rideRequestModel = RideRequestModel.fromMap(data.data);
    riderModel = await _riderServices.getRiderById(rideRequestModel.userId);
    notifyListeners();
